 ...
 * @mixin think\Model
 */
class ScreenshotPicture extends Model
{
    // 当你的数据表没有前缀的时候，name和table属性的定义是没有区别的，定义任何一个即可
	protected $name = 'ScreenshotPicture';// 模型名
    protected $table = 'think_screenshot_pictures';// 表名
	protected $pk = 'id';// 主键

    public function get_all($where,$order='id DESC',$total=10){
        $list = ScreenshotPicture::where($where)
                    ->order($order)
                    ->paginate([
                        'list_rows'=> $total,
                        'query' => Request::param()
 ...
 //        $count = ScreenshotPicture::where($where)->count();
 //        $list = ScreenshotPicture::where($where)
 //                    ->order($order)
 //                    ->page($p,$total)
 //                    ->select();
